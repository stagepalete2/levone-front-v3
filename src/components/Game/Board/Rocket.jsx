...
// ПРОВЕРЬТЕ ИМЯ ФАЙЛА ХЭНДЛЕРА! Если файл playgame.handler.js, то путь должен быть соответвующий:
import usePlayGame from '../../../api/handlers/game/rocketgame.handler'
import { useClient, useParams, useView } from '../../../zustand'
import styles from './Rocket.module.scss'

// Конфигурация уровней
const PRIZE_LEVELS = [
    { altitude: 200, label: '+100', icon: '🪙' },
    { altitude: 500, label: '+500', icon: '🪙' },
    { altitude: 1000, label: '+1000', icon: '🪙' },
...
// Переименовал в Rocket для порядка
const Rocket = () => {
    ...
    // Должно совпадать с CSS .rocketContainer bottom
    ...
    const handleLaunch = async () => {
        ...
        if (response.type === 'prize') {
            ...
            if (amount >= 1500) targetAltitude = 1200;
            else if (amount >= 1000) targetAltitude = 1000;
            else if (amount >= 500) targetAltitude = 600;
            else targetAltitude = 300;
        }
